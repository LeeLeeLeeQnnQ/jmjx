    ...
    <Card shadow style="margin-top: 5px;">
      <Tabs>
        <TabPane label="电表">
          <Card shadow>
            <Row type="flex" justify="start" align="middle" :gutter="20">
              <i-col v-for="item in energy_store_record_list" span="3">
                <Button style="margin: 5px;">{{item.meter_name}}</Button>
              </i-col>
            </Row>
          </Card>
          <Card shadow>
            <p slot="title">
                {{current_energy_store_name}}
            </p>
        ...
        <TabPane label="水表">
          <Card shadow>
            <Row type="flex" justify="start" align="middle" :gutter="20">
              <i-col v-for="item in water_store_record_list" span="3">
                <Button style="margin: 5px;">{{item.meter_name}}</Button>
              </i-col>
            </Row>
          </Card>
...
export default {
  ...
  data () {
    return {
      ...
      // 电表数据
      // 电表档口名单
      current_energy_store_name:'请选择抄表单位',
      energy_store_record_list:[],
      energy_record_list:[],
      energy_page: {
        current_page: 1,
        last_page: '',
      ...
      // 电表表格头
      current_water_store_name:'请选择抄表单位',
  ...
  methods: {
    ...
    initData( data ){
      ...
      this.energy_store_record_list = [];
      this.water_store_record_list = [];
      // 获取抄表列表
      queryMeterList( data ).then(res => {
        ...
        this.energy_store_record_list = arr1;
        this.water_store_record_list = arr2;
      })
    },
  },
  created () {
