<template>
  <div>
    ...
    <Tabs :animated="false" style="margin-top: 5px;">
        <TabPane label="建档中" name="0">
          <tables
            ref="pre_build_shops_tables"
            v-model="pre_build_shops_list"
            :columns="pre_build_shops_columns"
            @data-edit="editPreBuildShopCard"
...
export default {
  ...
  data () {
    return {
      ...
      pre_build_shops_columns: [
        ...
        {title: '创建时间', key: 'create_time',sortable: 'custom'},
  ...
  methods: {
    ...
    // 获取起租中
    getPrebuildShops( kitchen_id , page , keyword){
      let info = { kitchen_id : kitchen_id , order: this.pre_build_sort_data.order , key : this.start_sort_data.key }
      if(!!page) {
        info.page = page;
      }
      if(!!keyword){
    ...
    getPrebuildShopsNewPage(page){
      // 获取新页-全部店铺
      this.getPrebuildShops( this.sreach_kitchen_id , page , this.keyword  );
    },
    // 表格排序
    prebuildShopsSortTables(sort_data){
      ...
      this.getPrebuildShops( this.sreach_kitchen_id , this.pre_build_shops_page.current_page );
    },
    // 编辑数据
    editPreBuildShopCard(){
      console.log(111)
      let id = params.row.store_id
      let kitchen_id = params.row.kitchen_id
      const route = {
        name: 'canvassPreBuildEdit',
        query: {
          id,
          kitchen_id
        }
