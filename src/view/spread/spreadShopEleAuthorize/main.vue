...
<script>
// 权限
// StoreEle/index,StoreEle/edit,StoreEle/auth
import { getStoreEleList , editStoreEle } from '@/api/spread'
...
export default {
  ...
  data () {
    return {
      columns: [
        ...
        {title: '商户Id', key: 'id'},
        {title: 'shop_id', key: 'shop_id'},
        {title: 'shop_name', key: 'shop_name'},
        {title: 'access_token', key: 'access_token'},
        {title: 'refresh_token', key: 'refresh_token'},
        {title: 'expires_in', key: 'expires_in'},
        {
          title: '编辑',
          key: 'handle',
          width :80,
  ...
  methods: {
    ...
    editStoreEle(){
      ...
      editStoreEle(this.shopInfo).then(res => {
    ...
    handleShopAuthorize(info){
      ...
      const href = "./api/StoreEle/auth?id="+ info.id;
      window.open(href, '_blank')
    },
    // 初始化
    init(data){
      let obj = Object.assign(data,this.sreach)
      getStoreEleList(data).then(res => {
        const dbody = res.data
        if (dbody.code != 0) {
          this.$Notice.warning({
            title: dbody.msg
